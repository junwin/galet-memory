#!/usr/bin/env python3
"""Copy a legacy Lucy sqlite-vec store into galet-memory's canonical vec0 schema."""
...
def _require_legacy_schema(conn: sqlite3.Connection) -> None:
    ...
    present = {
        row[1] for row in conn.execute("PRAGMA table_info(embedding_metadata)").fetchall()
    }
    missing_columns = _REQUIRED_LEGACY_METADATA_COLUMNS - present
    if missing_columns:
        raise ValueError(
            "embedding_metadata missing columns: " + ", ".join(sorted(missing_columns))
        )
...
def prepare_records(
    ...
    *,
    ...
    for legacy in records:
        ...
        document_id, verified, status = _resolve_document_id(
            legacy.source_metadata, trust_content_hash
        )
        resolved_model = model.strip() or str(
            legacy.source_metadata.get("embedding_model") or legacy.source_metadata.get("model") or ""
        )
        resolved_provider = provider.strip() or str(
            legacy.source_metadata.get("embedding_provider") or legacy.source_metadata.get("provider") or ""
        )
        result.append(
            PreparedRecord(
                old_id=legacy.id,
    ...
    *,
    ...
    try:
        with SqliteVecEmbeddingIndex(
            destination, sqlite_vec_extension_path=extension_path
        ) as index:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = migrate_vec0_canonical(
            Path(args.source), Path(args.destination), args.extension_path,
